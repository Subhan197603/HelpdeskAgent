...
| SEC-2026-003 | High-severity `js-yaml` and `nanoid` build-tool advisories              | High          | Resolved | Platform engineering | Resolved 2026-08-13 by locked transitive patches to `js-yaml` 4.3.1 and `nanoid` 3.3.17. Neither package is installed in the final web image. `pnpm audit --audit-level high` must pass.                                                                                        |
...
| GHSA-2v37-7h3g-55p8 / CVE-2026-67213 | `nanoid` 3.3.17                       | Vite/PostCSS build tooling only; absent from final web image                                      | Patched; SEC-2026-003 resolved                               |
